...
- return G[b]
** Keuzes/deelproblemen identificeren
...
#+BEGIN_SRC c
Method HK(V = {depot, 1, ..,n})
Maak een hashtabel Q, initieel leeg
best = ∞
forall 𝑘 ∈ {1, … , 𝑛} do // loop door alle depots
        // zie k als eindpunt van route, doe daar bij de afstand van k tot depot
        best = min{ best, HKrec(𝑉 − {𝑘}, 𝑘) + d(𝑘, depot) }
return best

// functie om afstand van route adress g tot depot te bereken door S lopend
Method HKrec(𝑆, 𝑔) // volg recursieve functie
if 𝑆, 𝑔 ∈ 𝑄 then return 𝑄(𝑆, 𝑔)
else
    if 𝑆 = ∅ then return 𝑑 𝑑𝑒𝑝𝑜𝑡, 𝑔
    else 𝑎𝑛𝑡𝑤 = ∞;
        for 𝑘 ∈ 𝑆 do
            // recursieve call, afstand van k door de rest plus afstand tussen k en g
...
Reken alle 𝐴(𝑆, 𝑔) uit:
- Eerst alleen 𝑆 = ∅
- Dan alle 𝑆 met één adres
- Dan alle 𝑆 met twee adressen
- Dan alle 𝑆 met drie adressen
- Etc

Of representeer 𝑆 als integer …
...
*** Complexiteit
- Tijd: we kijken naar alle deelverzamelingen: 2𝑛
- Per verzameling 𝑛 keuzes voor laatste adres
- Per combinatie 𝑛 tijd, want kijken naar elk een-na- laatste adres:
- $O(2^n n^2)$
- Generalisatie: bijvoorbeeld: hoeveel adressen kan je binnen 𝑘 tijd bezoeken? (openingstijd van een winkelgebied?)
* Greedy algorithms
...
** Fractional Knapsack problem, Interval Scheduling en Huffman's codes
...
[[file:Deel_1/2024-05-09_16-02-26_screenshot.png]]


[[file:Deel_1/2024-05-09_16-02-53_screenshot.png]]
...
- $\pi [v]$: the predecessor of v when we traverse the graph
** Breadth-First Search: unweighted graph or graph with uniform weight
...
[[file:Single_source_shortest_path/2024-06-28_20-03-44_screenshot.png]]
...
[[file:Single_source_shortest_path/2024-06-28_20-33-07_screenshot.png]]
** Weighted DAG single source shortest path
[[file:Single_source_shortest_path/2024-06-28_20-34-45_screenshot.png]]

[[file:Single_source_shortest_path/2024-06-28_20-40-40_screenshot.png]]

Also time complexity of $O(|V|+|E|)$
** Dijkstras algorithm: weigthed graph with non-negagive weight
It can be looked at as bfs but by replacing the normal queue with a priority queue

[[file:Single_source_shortest_path/2024-06-28_20-54-16_screenshot.png]]

Time complexity: $O(|V|^2)$, this can be improved to $O(|E|+|V|\log|V|)$
** Bellman-Ford: weigthed graph without negative cycle
[[file:Single_source_shortest_path/2024-06-28_21-07-59_screenshot.png]]

Time complexity is $O(|V||E|)$: There is no shortest path with more than |V | − 1 edges. Therefore, the Bellman-Ford algorithm takes at most |V | − 1 rounds, and each round takes at most |E| checking and relaxation.
...
[[file:All_pair_shortest_paths/2024-06-29_13-00-26_screenshot.png]]
...
[[file:All_pair_shortest_paths/2024-06-29_10-57-16_screenshot.png]]
...
[[file:All_pair_shortest_paths/2024-06-29_11-20-06_screenshot.png]]
...
[[file:All_pair_shortest_paths/2024-06-29_11-43-08_screenshot.png]]
...
[[file:All_pair_shortest_paths/2024-06-29_12-02-54_screenshot.png]]
...
[[file:All_pair_shortest_paths/2024-06-29_12-55-30_screenshot.png]]

[[file:All_pair_shortest_paths/2024-06-29_12-55-53_screenshot.png]]

After calculating the h-values we can reweight all edges, run dijkstra and finally return the weights to the old value.

[[file:All_pair_shortest_paths/2024-06-29_12-58-01_screenshot.png]]
* Matchings
chapter 25
...
- $P(X=k)=(1-p)^{k-1}p$
- $E[X]=\frac 1 p$
** TODO Minimum cut probleem met gerandomiseerd algoritme
